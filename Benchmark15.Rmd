---
title: "Benchmark na 15 min podatkih"
author: "Tomas Rode"
date: "7 4 2022"
...
OPOZORILO: TA DOKUMENT SAMO PRIKAZUJE, DA UČENJE NA SAMIH 15 MINUTNIH PODATKIH NI MOGOČE (ČE ŽELIMO PREVERJATI KOT SMO IZBRALI -- ZA CELO LETO NAZAJ), SAJ JE ENOSTAVNO PREMALO PODATKOV ZA OCENJEVANJE MODELOV
...
Tukaj preizkušam model na samih 15minutnih podatkih.
...
```{r}
data_all_15$ELCE %>% msts( seasonal.periods = c(4*24, 4*24*7, 4*24*365)) %>% mstl() %>% autoplot()
```
...
splits <- initial_time_split(data_all_15, prop = 0.9)
...
actual_tbl <- data_all_15 %>% tail(4*150*24) %>%
...
```{r}
data_cv <- data.frame(data_all_15) %>% time_series_cv(initial = "1 year, 4 months", assess = "1 month", skip="1 month")
```
